...
public class MemberService {
    ...
    public RsData<Member> join(String username, String password ,String email , String birth,String phoneNumber) {
        // "GRAMGRAM" 해당 회원이 일반회원가입으로 인해 생성되었다는걸 나타내기 위해서
        return join("copyrightStreet", username, password,email,birth,phoneNumber);
    }

    // 내부 처리함수, 일반회원가입, 소셜로그인을 통한 회원가입(최초 로그인 시 한번만 발생)에서 이 함수를 사용함
    private RsData<Member> join(String providerTypeCode, String username, String password, String email, String birth,String phoneNumber) {
